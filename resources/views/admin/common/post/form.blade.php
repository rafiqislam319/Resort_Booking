...
@section('page-content')
   @include('admin.common.post.tab')
    @if(request()->get('custom-field'))
        @if(!empty($post))
        <form role="form" method="POST" action ="{{ route('admin_term_type_custom_field_data_store', $term_slug) }}" enctype="multipart/form-data">
        @csrf
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                    ...
                    */ ?>
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="pad">
                                <textarea xid="compose-textarea" class="form-control compose-textarea-summernote" name="description">{{ (!empty($post)) ? $post->description : '' }}</textarea>
                            </div>
                        </div>
                    @endif
          	...
          	<?php
          		$customField = \App\Models\PostField::where('term_type', $term_slug)->get();
          	?>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-sm {{ (!empty($post)) ?  : '' }}" id="{{ $term_slug }}_meta_title" name="meta_title" placeholder="Enter {{$term_name}} Meta Title" value="{{ (!empty($post)) ? $post->meta_title : '' }}" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label for="{{ $term_slug }}_meta_description">{{$term_name}} Meta Description</label>
                        <input type="text" class="form-control form-control-sm {{ (!empty($post)) ?  : '' }}" id="{{ $term_slug }}_meta_description" name="meta_description" placeholder="Enter {{$term_name}} Meta Description" value="{{ (!empty($post)) ? $post->meta_description : '' }}" autocomplete="off"}>
                    </div>
                    <div class="form-group">
                        <label for="{{ $term_slug }}_meta_keyword">{{$term_name}} Meta Keyword</label>
                        <input type="text" class="form-control form-control-sm {{ (!empty($post)) ?  : '' }}" id="{{ $term_slug }}_meta_keyword" name="meta_keyword" placeholder="Enter {{$term_name}} Meta Keyword" value="{{ (!empty($post)) ? $post->meta_keyword : '' }}" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label for="{{ $term_slug }}_meta_author">{{$term_name}} Meta Author</label>
                        <input type="text" class="form-control form-control-sm {{ (!empty($post)) ?  : '' }}" id="{{ $term_slug }}_meta_author" name="meta_author" placeholder="Enter {{$term_name}} Meta Author" value="{{ (!empty($post)) ? $post->meta_author : '' }}" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card"> <!-- category -->
                ...
                <div class="card-body">
                    ...
                    <div class="select2-dark">
                        <select id="{{$taxonomy->slug}}select" multiple class="form-control form-control-sm select2Cat" id="category_id" name="category_id[]">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="{{ $term_slug }}_order_by">{{$term_name}} Order</label>
                      	<input type="number" class="form-control form-control-sm" id="{{$term_slug}}_order_by" name="order_by" placeholder="Enter {{$term_name}} order" value="{{ (!empty($post)) ? $post->order_by : '' }}">
                  	</div>
                    ...
                    <div class="form-group">
                        ...
                        @endphp
                        <select name="is_status" id="{{$term_slug}}_is_status" class='form-control form-control-sm'>
