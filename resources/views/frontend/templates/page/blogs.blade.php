...
{{--                                    <figcaption class="figure-caption">Trulli, Puglia, Italy.</figcaption>--}}
                            ...
                            <?php
                            $getBlogLocale = new \App\Helpers\locale();
                            ?>

                        <div class="content-area">
                            <div class="date text-muted">{{$item->created_at}}</div>
                            <div class="title mt-3">
{{--                                <h2>{{ $item->name  }}</h2>--}}
                                <h2>{{ $getLang() == 'en' ? $item->name : $Model('Post')::customField('blog_title_bangla', $item->id)  }}</h2>
                            </div>
                            <div class="blog-short mb-3">

{{--                                <p>{{$Model('Post')::customField('blog_sub_title', $item->id)}}</p>--}}
                                <p>{{ $getLang() == 'en' ? $Model('Post')::customField('blog_sub_title', $item->id) : $Model('Post')::customField('blog_sub_title_bangla', $item->id) }}</p>
                            </div>

                            <div class="link">
                                <a href="{{route('frontend_page', [$item->term_type, $item->slug])}}" class="btn btn-sm">{{ $getBlogLocale->blog('read_more', $getLang()) }}</a>
                            </div>
                        </div>
                    </div>
                    @endforeach
